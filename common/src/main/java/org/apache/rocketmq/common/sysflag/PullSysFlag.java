...
public class PullSysFlag {
    private final static int FLAG_COMMIT_OFFSET = 0x1 << 0; // 表示从内存中读取的offset大于0
    private final static int FLAG_SUSPEND = 0x1 << 1; // 表示消息拉取时支持挂起
    private final static int FLAG_SUBSCRIPTION = 0x1 << 2; // 消息过滤机制为表达式
    private final static int FLAG_CLASS_FILTER = 0x1 << 3; // 消息过滤机制为类过滤模式
